* AURUM — reservations.js (نهائي مع تصحيح الإلغاء) */
...
/* ── Cancel booking – بعد الإلغاء، نعيد تحميل القائمة من الخادم ── */
...
// إعادة تحميل القائمة بالكامل من الخادم لضمان اتساق البيانات
await load();
...
<div class="res-state"> <h3>No reservations yet</h3> <p>Book a hotel to see it here.</p> <a class="btn-ghost" href="index.html?page=home">Discover hotels</a> </div>`; return; }
...
<article class="res-card"> <div class="res-card-art" style="background:url('${cover}') center/cover no-repeat;"> <div class="res-card-art-ref">${escapeHtml(b.reference)}</div> </div> <div class="res-card-body"> <h2 class="res-hotel-name">${escapeHtml(b.hotelName)}</h2> <div class="res-room-row"> <img class="res-room-img" src="${roomImg}" alt="${b.roomType}" loading="lazy"/> <span>${b.roomType} · ${b.rooms} room(s) · ${b.guests} guest(s)</span> </div> <div class="res-detail-grid"> <div><div class="res-detail-label">Check in</div><div>${fmtDate(b.checkIn)}</div></div> <div><div class="res-detail-label">Check out</div><div>${fmtDate(b.checkOut)}</div></div> <div><div class="res-detail-label">Nights</div><div>${b.nights}</div></div> <div><div class="res-detail-label">Booked</div><div>${fmtDate(b.createdAt)}</div></div> </div> </div> <div class="res-card-side"> <div class="res-total">${fmtMoney(b.total)}</div> ${b.paymentLast4 ? `<div style="font-size:10px">•••• ${b.paymentLast4}</div>` : ''} ${!isCancelled ? `<button class="btn btn-danger cancel-btn" data-id="${b.id}">Cancel</button>` : '<span class="cancelled-badge">Cancelled</span>' } </div> </article>`; }).join(''); }

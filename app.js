/* ═══════════════════════════════════════════════
   AURUM — app.js (FULLY CORRECTED - NO DUPLICATES)
   Hotels, images, AI concierge, booking badges, 1-day alerts
═══════════════════════════════════════════════ */
...
navLinks.forEach(link => {
  link.addEventListener('click', e => {
    ...
    if (link.getAttribute('href') === 'owner.html') {
      ...
      if (!u || u.role !== 'owner') showSideSigninTip(link, null, 'Sign in as owner');
...
const ROOM_IMGS = {
  'Deluxe Room':        'https://images.unsplash.com/photo-1631049307264-da0ec9d70304?w=1200',
  'Junior Suite':       'https://images.unsplash.com/photo-1591088398332-8a7791972843?w=1200',
  'Grand Suite':        'https://images.unsplash.com/photo-1582719508461-905c673771fd?w=1200',
...
function makeRealPhotos(hotelName, initial, color) {
  ...
  const labels = ['Lobby', 'Terrace', 'Courtyard'];
  return {
    hotel: [
      { url: cover || '', gradient: `linear-gradient(135deg,${c1},#2a1f0a)`, label: 'Exterior', initial },
      ...gallery.map((url, i) => ({ url, gradient: `linear-gradient(135deg,${c1},#0e0c06)`, label: labels[i] || 'View', initial })),
    ],
...
async function loadHotelsFromAPI() {
  ...
  } catch (e) { console.warn('API failed, using local fallback', e); }
  useLocalHotelDatabase();
}

function useLocalHotelDatabase() {
  hotelDatabase = [
    { id:1, name:'Le Grand Aurum Paris', city:'Paris', country:'France', stars:5, price:1850, rating:4.9, reviews:1284, desc:'Belle Époque grandeur...', amenities:['Wi-Fi','Spa','Restaurant','Concierge'], initial:'LG', color:'#1a1208', maxChildren:4, rooms:5, imageUrl:HOTEL_COVERS['Le Grand Aurum Paris'], photos:makeRealPhotos('Le Grand Aurum Paris','LG','#1a1208') },
    { id:2, name:'Aurum Palace London', city:'London', country:'UK', stars:5, price:2200, rating:4.8, reviews:876, desc:'Georgian townhouse...', amenities:['Wi-Fi','Pool','Spa','Concierge'], initial:'AP', color:'#14100a', maxChildren:2, rooms:5, imageUrl:HOTEL_COVERS['Aurum Palace London'], photos:makeRealPhotos('Aurum Palace London','AP','#14100a') },
    { id:3, name:'Aurum Royale Dubai', city:'Dubai', country:'UAE', stars:5, price:2500, rating:4.9, reviews:2341, desc:'Beachfront gold leaf...', amenities:['Private Beach','Infinity Pool','Butler','Helipad'], initial:'AR', color:'#1a1008', maxChildren:4, rooms:4, imageUrl:HOTEL_COVERS['Aurum Royale Dubai'], photos:makeRealPhotos('Aurum Royale Dubai','AR','#1a1008') },
    { id:4, name:'Aurum Zen Bali', city:'Bali', country:'Indonesia', stars:5, price:1100, rating:4.88, reviews:789, desc:'Jungle sanctuary...', amenities:['Jungle Pool','Spa','Butler','Yoga'], initial:'AZ', color:'#0e1208', maxChildren:3, rooms:3, imageUrl:HOTEL_COVERS['Aurum Zen Bali'], photos:makeRealPhotos('Aurum Zen Bali','AZ','#0e1208') },
    { id:5, name:'Aurum Riviera Santorini', city:'Santorini', country:'Greece', stars:5, price:2400, rating:4.95, reviews:432, desc:'Cliffside villas...', amenities:['Infinity Pool','Cave Spa','Butler','Sunset Dining'], initial:'AS', color:'#0e0818', maxChildren:2, rooms:2, imageUrl:HOTEL_COVERS['Aurum Riviera Santorini'], photos:makeRealPhotos('Aurum Riviera Santorini','AS','#0e0818') },
    { id:6, name:'Aurum Bosphorus Istanbul', city:'Istanbul', country:'Turkey', stars:5, price:1700, rating:4.8, reviews:567, desc:'Ottoman palace...', amenities:['Hammam','Rooftop Pool','Butler','Yacht'], initial:'AB', color:'#1a1208', maxChildren:2, rooms:3, imageUrl:HOTEL_COVERS['Aurum Bosphorus Istanbul'], photos:makeRealPhotos('Aurum Bosphorus Istanbul','AB','#1a1208') },
  ];
  window._hotelsData = hotelDatabase;
}
...
document.getElementById('searchBtn').addEventListener('click', () => {
  ...
  if (!loc) { showToast('Enter destination', 'error'); return; }
...
function renderResults(hotels, loc, rooms, children, price) {
  ...
  meta.textContent = `Showing ${hotels.length} properties · ${rooms} room(s) · ${children} child(ren) · ${pl}`;
  grid.innerHTML = '';
  if (!hotels.length) {
    grid.innerHTML = '<div class="no-results">No properties found.<br/><small>Try adjusting filters.</small></div>';
    return;
  }
  hotels.forEach((h, i) => {
  ...
  card.innerHTML = `
    ...
    <div class="hotel-card-body">
      ...
      <div class="hotel-card-footer">
        ...
        <div style="text-align:right"><span class="stars">${stars}</span><span class="rating-count">${hotel.rating} (${hotel.reviews?.toLocaleString() || 0})</span></div>
      </div>
...
/* ══════════ GALLERY MODAL (unchanged but works) ══════════ */
...
function closeGallery() { galleryModal.classList.remove('open'); document.body.style.overflow = ''; const tip = document.getElementById('signinTip'); if(tip) tip.remove(); }
...
document.getElementById('openAiChat')?.addEventListener('click', () => { aiModal.classList.add('open'); setTimeout(()=>aiInput.focus(),100); loadChatHistory(); });
document.getElementById('aiFab')?.addEventListener('click', () => { aiModal.classList.add('open'); setTimeout(()=>aiInput.focus(),100); loadChatHistory(); });
...
document.getElementById('aiClear')?.addEventListener('click', () => {
  if(confirm('Clear chat?')) { window._aiHistory = []; localStorage.removeItem(getChatStorageKey()); loadChatHistory(); }
});
// Minimize & dragging (preserved from earlier but simplified)
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if(currentUser) { await fetchUserBookings(); checkUpcomingBookings(); updateAllBookingBadges(); }
  const navToggle = document.getElementById('navToggle');
  if(navToggle) navToggle.addEventListener('click', () => document.querySelector('.nav-links')?.classList.toggle('mobile-open'));
  loadChatHistory();
  ...
  // Preload featured clicks
  document.querySelectorAll('.featured-card').forEach(card => {
    if(!card.getAttribute('data-listener')) {
      card.setAttribute('data-listener','true');
      card.addEventListener('click', () => {
        const dest = card.dataset.dest;
        if(dest) { document.getElementById('s-location').value = dest; document.getElementById('searchBtn').click(); }
      });
    }
  });
});

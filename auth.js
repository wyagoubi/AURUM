/* AURUM — auth.js (Backend API based with reliable token storage) */
...
function setError(id, msg) {
  ...
  if (el) { el.textContent = msg; if (msg) setTimeout(() => { if (el) el.textContent = ''; }, 4000); }
}
function clearError(id) { setError(id, ''); }
function highlightInvalid(inputId) {
  const el = document.getElementById(inputId);
  if (el) { el.style.borderColor = '#e74c3c'; setTimeout(() => { if (el) el.style.borderColor = ''; }, 2000); }
}
  ...
  if (!overlay) {
    ...
    overlay.innerHTML = `
      <div class="success-content">
        <div class="success-checkmark"><svg viewBox="0 0 50 50" width="50" height="50"><circle cx="25" cy="25" r="23" fill="none" stroke="currentColor" stroke-width="2" class="checkmark-circle"/><path d="M14 27l7 7 15-15" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="checkmark-check"/></svg></div>
...
if (new URLSearchParams(window.location.search).get('role') === 'owner') switchRole('owner');
...
function switchOwnerTab(tab) {
  if (!ownerLoginWrap || !ownerRegWrap) return;
  if (tab === 'login') {
...
window.togglePw = function (id, btn) {
  ...
  if (btn) btn.style.opacity = inp.type === 'password' ? '0.5' : '1';
};
...
function applyStrength(val, fillId, labelId) {
  ...
  const map = [{ w: '20%', bg: '#e74c3c', txt: 'Weak' }, { w: '45%', bg: '#e67e22', txt: 'Fair' }, { w: '70%', bg: '#f1c40f', txt: 'Good' }, { w: '100%', bg: '#2ecc71', txt: 'Strong' }];
...
function showMsg(text, type) {
  const el = document.getElementById('authMsg') || (() => { const e = document.createElement('div'); e.id = 'authMsg'; e.style.cssText = 'position:fixed;bottom:32px;right:32px;z-index:9999;padding:14px 24px;font-size:12px;letter-spacing:0.5px;border:1px solid;max-width:340px;background:var(--bg2);'; document.body.appendChild(e); return e; })();
  el.textContent = text;
...
if (loginBtn) {
  loginBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      else console.warn('⚠️ No token received from server');
...
if (registerBtn) {
  registerBtn.addEventListener('click', async () => {
    ...
    if (!pass || pass.length < 6) { setError('regPassErr', 'Password min 6 characters'); ok = false; }
    if (pass !== pass2) { setError('regPass2Err', 'Passwords do not match'); ok = false; }
    if (!agreed) { setError('agreeErr', 'You must agree to terms'); ok = false; }
    if (!ok) return;
    ...
    try {
      ...
      localStorage.setItem('aurum-user', JSON.stringify(user));
      if (user.token) localStorage.setItem('aurum-token', user.token);
      showSuccessOverlay(`Welcome, ${first}!`, 'Setting up your account...');
      setTimeout(() => { window.location.href = 'index.html'; }, 1500);
    } catch (err) {
...
if (ownerRegisterBtn) {
  ownerRegisterBtn.addEventListener('click', async () => {
    ...
    if (!pass || pass.length < 6) { setError('ownerPassErr', 'Password min 6 characters'); ok = false; }
    if (!agreed) { setError('ownerAgreeErr', 'You must accept terms'); ok = false; }
    if (!ok) return;
    ...
    try {
      ...
      localStorage.setItem('aurum-user', JSON.stringify(user));
      if (user.token) localStorage.setItem('aurum-token', user.token);
      showSuccessOverlay(`Welcome, ${first}!`, 'Setting up your dashboard...');
      setTimeout(() => { window.location.href = 'owner-dashboard.html'; }, 1500);
    } catch (err) {
